  ...
  return (
    <>
        <div className=' flex flex-col md:flex-row gap-5 w-full justify-center items-center md:justify-around'>
          {cardItems.map((item) => (
            <div key={item.id} className=' flex gap-5 flex-col justify-center items-center w-[250px]'>
              <div>
              ...
              <div className=' flex flex-col gap-2'>
                ...
                <div className=' text-center'><h3 className=' text-[#626E79] font-bold'>{item.head}</h3></div>
                <div><p className=' text-[#626E79]'>{item.desc}</p></div>
                <div className=' flex items-baseline gap-2'>
                  <p className=' text-[#5C6874] text-[20px]'>{item.discountPrice}</p>
                  <p className=' text-[#9FA9B3] text-[14px]'>{item.originalPrice}</p>
                  <p className=' text-[#EF4C5D] text-[13px]'>{item.off}</p>
                </div>
              </div>
              <div className=''>
                <Btn btnName={item.btnName} width={"250px"}/>
              </div>
            </div>
          ))}
        </div>
