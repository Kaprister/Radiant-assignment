        ...
        <div className=' flex flex-col gap-16 px-5'>
            {cards.map((card) => (
                ...
                >
                    ...
                    <div className={`absolute top-0 left-0 flex justify-center items-center ${card.id === 1 ? "" : "border"} border-[#E1E4E6] w-[40px] h-[40px] rounded-full text-[#626E79]`} ><p>{card.id}</p></div>
                    ...
                    <div className=' w-full md:w-[50%] flex flex-col gap-2 justify-start'>
                        <div className=' text-[#4B5665]'>
                            <span className=' text-[#2C384A] text-[16px] font-bold'>{card.spanHead}</span>
                            {card.content}
                        </div>
                        <h2 className=' text-[#2C384A] text-[16px] font-bold'>{card.subhead}</h2>
                        <div className={` pl-5 ${card.id === 4 ? "hidden" : ""} text-[#4B5665]`}>{card.desc}</div>
                            {card.descArr &&
                                <div className=' flex flex-col gap-4'>
                                    <div className={` ml-6 flex flex-col gap-2 text-[#4B5665] bg-[#FFF4ED] p-2 mr-5 rounded-lg`}>
                                        {card.descArr.map((item, index) => (
                                            <div key={index} className=' text-[#4B5665] flex gap-3'>
                                                <div className=' w-[30px] h-[24px] bg-white rounded-md text-[#1B88F4] text-center'>{item.sn}</div>
                                                <div>{item.name}</div>
                                            </div>
                                        ))}
                                    </div>
                                    <div className=' flex flex-col gap-3'>
                                        <p className=' text-[#2C384A] font-medium'>Why we love it</p>
                        ...
                        <div className=' flex items-center'>
                            <p className=' cursor-pointer text-[#1B88F4]'>{card.showMore}</p>
                    ...
                    <div className=' w-full md:w-[25%] flex flex-col justify-between gap-10 items-center'>
                        <div>
                            <ReviewTag rating={card.rating} grade={card.grade} cid={card.id}/>
                        </div>
                        <div>
                            <Btn btnName={"view"} width={"190px"}/>
                        </div>
                    </div>
                </div>
            ))}
