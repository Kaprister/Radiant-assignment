      ...
      <nav className='flex flex-col md:flex-row gap-5 md:gap-10 justify-around md:justify-center items-center bg-[#212731] p-4 text-white'>
        <div className=' flex gap-5'>
            ...
            <div className=' md:hidden'>
              <img className={` ${!isMenuOpen ? "" : "hidden"}`}  onClick={() => setIsMenuOpen(true)} src={Menu} alt='ham-icon'/>
              <img className={` ${isMenuOpen ? "" : "hidden"}`} onClick={() => setIsMenuOpen(false)} src={closeMenu} alt='ham-icon'/>
            </div>
        </div>
        <div className={` md:flex flex-col md:flex-row gap-5 md:gap-10 ${isMenuOpen ? "flex" : "hidden"}`}>
